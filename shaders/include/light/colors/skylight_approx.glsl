...
vec3 skylight_approx(vec3 normal, vec3 flat_normal, vec3 shadows, float directional_lighting, float ao) {
	...
	// Night sky light so night isn't pitch black
    vec3 night_ground_tint = vec3(0.13, 0.13, 0.13) * (NIGHT_SKY_LIGHT_I * 2);
    float night_factor = 1.0 - smoothstep(0.0, 0.5, sun_dir.y);

    skylight += night_ground_tint * night_factor * 0.2;
